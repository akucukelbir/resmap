...
if __name__ == '__main__':
	...
	Mbegin = round((2.1*vxSize)/0.5)*0.5	# round to the nearest 0.5
	Mstep  = 0.5
	M      = Mbegin
	...
	mask         = np.bitwise_and(dataMask, R < n/2 - 2)
	...
	dataSpect = sphericalAverage(dataFabs) + 1e-10
	m, s      = divmod(time() - tStart, 60)
	print "  :: Time elapsed: %d minutes and %.2f seconds" % (m, s)
	...
	wpoly = np.array(np.bitwise_and(xpoly>n/4, xpoly<n/2-n/20), dtype='float32')
	...
	dataPWSpect = sphericalAverage(dataPWFabs) + 1e-10
